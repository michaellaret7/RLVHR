...
def train(
    ...
    for epoch in range(config.num_epochs):
        ...
        for prompt in epoch_prompts:
            ...
            all_completions = []      # answer only, as text (for scoring)
            all_sequences = []        # prompt + answer, as token IDs (to re-run for gradients)
            all_prompt_lengths = []   # where the prompt ends in each sequence (to build the mask)
            ...
            with torch.no_grad():
                ...
                # Generate completions for the current prompt (G times) and store the results in the lists initialized above
                for _ in range(config.generations_per_prompt):
                    # Generate a completion for the current prompt from the LLM
                    ...
                    full_sequence = outputs[0] # The full sequence of tokens generated by the LLM prompt + completion
                    completion_only = full_sequence[prompt_length:] # Set the completion only tokens as a variable

                    completion_text = tokenizer.decode( # The completion text converted from the completion only tokens
                        completion_only, 
                        skip_special_tokens=True
                    )

                    # Append the results of the completion to the lists
